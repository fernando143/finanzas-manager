...
export class MercadoPagoService {
  ...
  async getOAuthToken(): Promise<MercadoPagoToken> {
    ...
    if (this.tokenCache && this.tokenCache.expiresAt > new Date()) {
      return {
        ...
        token_type: 'Bearer',
        expires_in: Math.floor((this.tokenCache.expiresAt.getTime() - Date.now()) / 1000),
        scope: 'read write'
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`MercadoPago OAuth failed: ${response.status} ${response.statusText}. ${errorText}`)
      }
      ...
      // Cache the token
      const expirationTime = new Date(Date.now() + (validatedData.expires_in - 300) * 1000) // Subtract 5 minutes for safety
      this.tokenCache = {
      ...
      throw new Error(`Failed to obtain MercadoPago OAuth token: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async searchPayments(options: {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text()
        throw new Error(`MercadoPago payments search failed: ${response.status} ${response.statusText}. ${errorText}`)
      }
      ...
      throw new Error(`Failed to search MercadoPago payments: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  filterExpensePayments(payments: MercadoPagoPayment[]): MercadoPagoPayment[] {
    return payments.filter(payment => {
      ...
      const validOperationTypes = ['money_transfer', 'regular_payment', 'payment']
      if (!validOperationTypes.includes(payment.operation_type)) {
        return false
      }

      // Exclude refunds and other non-expense operations
      if (payment.status_detail && ['refunded', 'cancelled', 'rejected'].includes(payment.status_detail)) {
        return false
      }
   ...
   */
  convertPaymentToExpense(payment: MercadoPagoPayment, userId: string, defaultCategoryId: string) {
   ...
   */
  async getRecentExpensePayments(userId: string, defaultCategoryId: string, days: number = 30) {
    ...
    try {
      ...
      const searchResponse = await this.searchPayments({
        dateFrom,
        dateTo: new Date(),
        limit: 100 // Get up to 100 recent payments
      })

      // Filter to only expense-type payments
      const expensePayments = this.filterExpensePayments(searchResponse.results)
