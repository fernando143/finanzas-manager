...
class HttpClient {
  ...
  private setupInterceptors(): void {
    ...
    this.axiosInstance.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        console.error(`❌ ${error.response?.status || 'Network'} ${originalRequest?.method?.toUpperCase()} ${originalRequest?.url}:`, error.message)
        return Promise.reject(error)
      }
    )
  }
   ...
   * GET request
   */
  async get<T>(endpoint: string, query?: Record<string, unknown>): Promise<ApiResponse<T>> {
    try {
      ...
      if (response.data.success) {
        return this.createResponse(response.data.data as T, response.data.message)
      } else {
        return this.createErrorResponse(response.data.error || 'Request failed')
      }
    } catch (error) {
